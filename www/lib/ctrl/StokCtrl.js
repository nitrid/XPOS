    ...
    function StokListeGetir()
    {
        ...
        for (let x = 0; x < $scope.StokListesi.Kolon.length; x++) 
        {
            ...
            if($scope.StokListesi.Kolon[x].CODE == "PRICE")    
            {
                ...
                QueryField.Price.Outer = "LEFT OUTER JOIN ITEM_PRICE ON ITEM_PRICE.ITEM_CODE = ITEMS.CODE AND ITEM_PRICE.TYPE = 0 AND ((ITEM_PRICE.FINISH_DATE > '19700101' AND ITEM_PRICE.FINISH_DATE > GETDATE()) OR (ITEM_PRICE.FINISH_DATE = '19700101')) ";
            } 
