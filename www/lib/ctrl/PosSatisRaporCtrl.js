    ...
    {        
        ...
        let TmpQuery = 
        {
            ...
            query:  "SELECT LUSER,COUNT(LUSER) AS [COUNT],0 AS TIP FROM (SELECT MAX(LUSER) AS LUSER FROM POS_SALES WHERE STATUS = 0 GROUP BY REF,REF_NO) AS TMP GROUP BY LUSER " +
                    "UNION ALL " +
                    "SELECT LUSER,COUNT(LUSER) AS [COUNT],1 AS TIP FROM (SELECT MAX(LUSER) AS LUSER FROM POS_SALES WHERE STATUS = -1 AND DOC_DATE >= CONVERT(NVARCHAR(10),GETDATE(),112) GROUP BY REF,REF_NO) AS TMP GROUP BY LUSER " +
                    "UNION ALL " +
                    "SELECT LUSER,COUNT(LUSER) AS [COUNT],2 AS TIP FROM (SELECT MAX(LUSER) AS LUSER FROM POS_SALES WHERE STATUS = -2 AND DOC_DATE >= CONVERT(NVARCHAR(10),GETDATE(),112) GROUP BY REF,REF_NO) AS TMP GROUP BY LUSER",
        }
