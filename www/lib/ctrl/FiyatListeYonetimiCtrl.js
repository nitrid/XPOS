function FiyatListeYonetimiCtrl ($scope,$window,db)
{
    ...
    $scope.BtnAra = async function()
    {
        ...
        if($scope.Kodu != '')
        {
            ...
            {
                TmpQuery.query = TmpQuery.query.replace('{0}'," ((ITEMS.CODE IN (" + TmpVal + ")) OR ITEMS.CODE IN (SELECT ITEM_CODE FROM ITEM_BARCODE WHERE BARCODE IN (" + TmpVal + ")) OR ITEM_CUSTOMER.CUSTOMER_ITEM_CODE IN (" + TmpVal + ")) AND ");
            }
            else
            {
                TmpQuery.query = TmpQuery.query.replace('{0}', " ((ITEMS.CODE LIKE " + TmpVal + " + '%') OR (ITEMS.CODE = (SELECT ITEM_CODE FROM ITEM_BARCODE WHERE BARCODE LIKE " + TmpVal + " + '%')) OR ITEM_CUSTOMER.CUSTOMER_ITEM_CODE LIKE " + TmpVal + " + '%') AND ");
            }
        }
        else
        {
